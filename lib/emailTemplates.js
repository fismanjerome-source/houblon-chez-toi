...
function orderConfirmationEmail({ order, user }) {
  ...
  const body = `
    ...
    <table role="presentation" width="100%" style="font-size:14px; margin-top:8px;">
      ...
      ${order.discountCents > 0 ? `<tr><td>Remise volume pro</td><td style="text-align:right;">− ${euros(order.discountCents)}</td></tr>` : ''}
      ${order.promoDiscountCents > 0 ? `<tr><td>Code ${order.promoCode || ''}</td><td style="text-align:right;">− ${euros(order.promoDiscountCents)}</td></tr>` : ''}
      ${order.creditUsedCents > 0 ? `<tr><td>Crédit utilisé</td><td style="text-align:right;">− ${euros(order.creditUsedCents)}</td></tr>` : ''}
    ...
    <p style="font-size:13.5px; color:rgba(15,23,18,0.7); margin-top:20px;">
      ${order.paymentChoice === 'STRIPE'
        ? 'Commande payée par carte, merci !'
        : order.paymentChoice === 'NET_30'
        ? 'En tant que compte pro, le règlement se fait par facture à 30 jours (facture envoyée après livraison).'
...
function invoiceEmail({ order, user, invoice }) {
  ...
  const body = `
    ...
    <table role="presentation" width="100%" style="font-size:14px; margin-top:8px;">
      ...
      ${order.creditUsedCents > 0 ? `<tr><td>Crédit utilisé</td><td style="text-align:right;">− ${euros(order.creditUsedCents)}</td></tr>` : ''}
      ${order.depositReturnedCents > 0 ? `<tr><td>Reprise de consignes</td><td style="text-align:right;">− ${euros(order.depositReturnedCents)}</td></tr>` : ''}
      <tr><td style="font-weight:bold; padding-top:8px;">${invoice.dueDate ? 'Total dû' : 'Total payé'}</td><td style="text-align:right; font-weight:bold; padding-top:8px;">${euros(order.totalCents)}</td></tr>
    </table>
    <p style="font-size:13.5px; color:rgba(15,23,18,0.7); margin-top:20px;">
      ${invoice.dueDate
        ? `À régler avant le ${invoice.dueDate.toLocaleDateString('fr-FR')} (facture à 30 jours, compte pro).`
        : order.paymentChoice === 'STRIPE'
        ? 'Réglée par carte au moment de la commande.'
        : 'Réglée en espèces à la livraison ou au retrait.'}
...
function referralInviteEmail({ user }) {
  const body = `
    <h1 style="font-size:20px; margin:0 0 4px;">Offrez 5 €, gagnez 10 € !</h1>
    <p style="font-size:14px; line-height:1.7; color:rgba(15,23,18,0.8);">
      Bonjour ${user.name.split(' ')[0]}, si Houblon chez toi vous plaît, pourquoi ne pas en faire profiter
      un proche ? Partagez votre code, votre ami économise <strong>5 €</strong> sur sa première commande,
      et dès qu'elle est livrée, vous recevez <strong>10 €</strong> de crédit sur votre prochaine commande.
    </p>
    <div style="text-align:center; margin:24px 0;">
      <div style="display:inline-block; background:${COLORS.paperWarm}; border:1px dashed ${COLORS.copper}; border-radius:6px; padding:14px 28px; font-family:'Courier New', monospace; font-size:20px; letter-spacing:0.08em; color:${COLORS.pine};">
        ${user.referralCode}
      </div>
    </div>
    <p style="font-size:13px; color:rgba(15,23,18,0.6); text-align:center;">
      Votre proche entre ce code au moment de sa commande, sur houbloncheztoi.fr.
    </p>
  `;
  return { subject: `Offrez 5 €, gagnez 10 € — Houblon chez toi`, html: baseLayout({ title: 'Parrainage', preheader: 'Partagez votre code et gagnez du crédit.', bodyHtml: body }) };
}

module.exports = { orderConfirmationEmail, invoiceEmail, reviewRequestEmail, referralInviteEmail, SITE_URL };
