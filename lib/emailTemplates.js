...
function invoiceEmail({ order, user }) {
  const lines = orderLines(order);
  const body = `
    <h1 style="font-size:20px; margin:0 0 4px;">Votre facture — commande n°${order.id.slice(-6)}</h1>
    <p style="font-size:14px; line-height:1.6; color:rgba(15,23,18,0.75);">
      Livrée le ${order.slot} ${order.pickup ? '(retrait à Bondues)' : `à ${order.town}`}.
    </p>
    ${linesTable(lines)}
    <table role="presentation" width="100%" style="font-size:14px; margin-top:8px;">
      ${order.deliveryFeeCents > 0 ? `<tr><td>Livraison</td><td style="text-align:right;">${euros(order.deliveryFeeCents)}</td></tr>` : ''}
  ...
  `;
  return { subject: `Votre facture — commande Houblon chez toi n°${order.id.slice(-6)}`, html: baseLayout({ title: 'Facture', preheader: 'Voici la facture de votre commande livrée.', bodyHtml: body }) };
}
