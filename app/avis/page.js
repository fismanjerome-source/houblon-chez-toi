...
export default async function AvisPage() {
  ...
  return (
    <main className="wrap" style={{ padding: '48px 0', maxWidth: 720 }}>
      ...
      {reviews.length > 0 ? (
        <p style={{ color: 'rgba(15,23,18,0.75)', fontSize: 15, marginBottom: 28 }}>
          <Stars rating={Math.round(average)} /> {average.toFixed(1)} / 5 sur {reviews.length} avis
        </p>
      ) : (
        <p style={{ color: 'rgba(15,23,18,0.65)' }}>Pas encore d'avis publiés — les premiers arrivent bientôt !</p>
      )}

      {reviews.map((r) => (
        <div key={r.id} style={{ border: '1px solid var(--line)', borderRadius: 6, padding: 16, marginBottom: 12, background: 'white' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6, flexWrap: 'wrap', gap: 8 }}>
            <strong style={{ color: 'var(--pine)', fontSize: 14.5 }}>{r.customerName}</strong>
            <Stars rating={r.rating} />
          </div>
          {r.comment && <p style={{ margin: 0, fontSize: 14, color: 'rgba(15,23,18,0.8)', lineHeight: 1.6 }}>{r.comment}</p>}
        </div>
      ))}
    </main>
  );
