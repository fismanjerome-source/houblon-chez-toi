  ...
  return (
    <section className="wrap" style={{ padding: '8px 0 40px' }}>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        {reviews.map((r) => (
          <div key={r.id} style={{ flex: '1 1 220px', border: '1px solid var(--line)', borderRadius: 6, padding: 16, background: 'white' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6, gap: 8 }}>
              <strong style={{ color: 'var(--pine)', fontSize: 14 }}>{r.customerName}</strong>
              <Stars rating={r.rating} />
            </div>
            {r.comment && <p style={{ margin: 0, fontSize: 13.5, color: 'rgba(15,23,18,0.8)', lineHeight: 1.6 }}>{r.comment}</p>}
          </div>
        ))}
      </div>
    </section>
