...
export default async function AdminEmailsPage() {
  ...
  return (
    <main className="wrap" style={{ padding: '48px 0' }}>
      ...
      <p style={{ fontSize: 13.5, color: 'rgba(var(--ink-rgb),0.6)', marginBottom: 24 }}>
        Aperçu en lecture seule, avec des données d'exemple, tels que les emails partent réellement.
      </p>

      {templates.map((t) => (
        <div key={t.label} style={{ marginBottom: 32, border: '2px solid var(--line)', borderRadius: 2, overflow: 'hidden', background: 'var(--surface)' }}>
          <div style={{ padding: '12px 16px', borderBottom: '2px solid var(--line)' }}>
